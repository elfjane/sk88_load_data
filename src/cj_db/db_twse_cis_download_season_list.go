...
// 取得最後一天
func DB_twse_cis_download_season_list_lastday() string {
...
// 新增要下載的日期
func DB_twse_cis_download_season_list_insert_init(dt_id string, csv_filename string) {
...
// 刪除星期日天數
func DB_twse_cis_download_season_list_delete_week() {
	Exec("DELETE FROM `stock`.`twse_download_day_list` WHERE DAYOFWEEK(`dt_id`) = 1")
}

// 取得要下載的列表
type Twse_cis_season_select_no_download struct {
	Dt_id        string `sql:"dt_id"`
	Csv_filename string `sql:"csv_filename"`
}

func DB_twse_cis_download_season_list_get() []Twse_select_no_download {
...
// 更新 download
func DB_twse_cis_download_season_list_download_update(dt_id string, file_size int64) {
	Exec("UPDATE `stock`.`twse_download_day_list` SET `csv_size` = ? , `is_download` = is_download + 1 WHERE `dt_id` = ?; ", file_size, dt_id)
}

// 更新 download
func DB_twse_cis_download_season_list_is_change_update(dt_id string) {
	Exec("UPDATE `stock`.`twse_download_day_list` SET `is_change` = is_change + 1 WHERE `dt_id` = ?; ", dt_id)
}

// 更新 download
func DB_twse_cis_download_season_list_is_insert_update(dt_id string) {
	Exec("UPDATE `stock`.`twse_download_day_list` SET `is_insert` = is_insert + 1 WHERE `dt_id` = ?; ", dt_id)
}

// 取得尚未讀取的列表列表
type Twse_cis_season_no_insert_list struct {
	Id        string `sql:"id"`
	Csv_filename string `sql:"csv_filename"`
}

func DB_twse_cis_download_season_no_insert_list() []Twse_cis_season_no_insert_list {
	...
	fmt.Println("query db start")
	var data_list = []Twse_cis_season_no_insert_list{}
	for results.Next() {
		// for each row, scan the result into our tag composite object
		var twse_cis_season_no_insert_list Twse_cis_season_no_insert_list
		err = results.Scan(&twse_cis_season_no_insert_list.Id,
			&twse_cis_season_no_insert_list.Csv_filename)
		if err != nil {
			panic(err.Error()) // proper error handling instead of panic in your app
		}

		data_list = append(data_list, twse_cis_season_no_insert_list)
	}
	results.Close()
	fmt.Println("query db success")
	return data_list
...
// 取得尚未讀取的列表列表
type Twse_cis_season_is_change_list struct {
	Id        string `sql:"id"`
	Csv_filename string `sql:"csv_filename"`
}

func DB_twse_cis_download_is_change_list() []Twse_cis_season_is_change_list {
	Connect()
	...
	fmt.Println("query db start")
	var data_list = []Twse_cis_season_is_change_list{}
	for results.Next() {
		// for each row, scan the result into our tag composite object
		var twse_cis_season_is_change_list Twse_cis_season_is_change_list
		err = results.Scan(&twse_cis_season_is_change_list.Id,
			&twse_cis_season_is_change_list.Csv_filename)
		if err != nil {
			panic(err.Error()) // proper error handling instead of panic in your app
		}

		data_list = append(data_list, twse_cis_season_is_change_list)
	}
	results.Close()
	fmt.Println("query db success")
	Close()
